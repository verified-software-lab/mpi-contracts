...
public class CommonTypeFactory implements TypeFactory {
	...
	@Override
	public SetType theSetType(Type elementType) {
		assert elementType.kind() != TypeKind.SET;
		return new CommonSetType(elementType);
	}
	
	@Override
	public TupleType tupleType(Type... componentTypes) {
		int numComps = componentTypes.length;

		for (Type compType : componentTypes)
			if (!compType.isScalar())
				throw new RuntimeException(
						"Tuple component type must be scalar but saw "
								+ compType);
		return new CommonTupleType(TypeKind.TUPLE, numComps,
				Arrays.asList(componentTypes));
	}

	@Override
	public AcslMPIBufType acslMPIBufType() {
		return new CommonAcslMPIBufType();
	}

	@Override
	public AcslMPISigType acslMPISigType() {
		return new CommonAcslMPISigType();
	}
	
	@Override
	public AcslMPISeqType acslMPISeqType() {
		return new CommonAcslMPISeqType();
	}
}
